...
describe("Paywall", () => {
  ...
  it("opens with every claim already readable — this is an archive, not a teaser", async () => {
    const { getByText } = await renderWithProviders(<Paywall />);
    ...
    expect(getByText(t("feat3Desc"))).toBeTruthy();
  });

  it("lets a claim be put away without touching the others", async () => {
  ...
  it("marks a put-away row as collapsed and a fresh one as expanded, for accessibility", async () => {
    const { getByLabelText } = await renderWithProviders(<Paywall />);
    const row = getByLabelText(t("feat1Title"));
  ...
  // rendered as an empty row. Quandary genuinely has three claims, not four.
